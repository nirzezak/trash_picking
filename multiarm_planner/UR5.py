...
class Robotiq2F85:
    def __init__(self, ur5, color, replace_textures=True):
        ...
        # gripper_loc = pose[0][:]
        # gripper_loc[1] -= 0.075
        # p.resetBasePositionAndOrientation(self.body_id, gripper_loc, p.getQuaternionFromEuler([-np.pi / 2, np.pi / 2, 0]))
...
class UR5:
    ...
    def close_gripper(self):
        if self.end_effector is not None:
            p.setJointMotorControl2(self.end_effector.body_id, 1, p.VELOCITY_CONTROL, targetVelocity=5, force=10000)
    ...
    def step_simulation(self, num_steps):
        for i in range(int(num_steps)):
            ...
            if self.end_effector is not None:
                # Constraints
                gripper_joint_positions = np.array([p.getJointState(self.end_effector.body_id, i)[
                                                0] for i in range(p.getNumJoints(self.end_effector.body_id))])
                p.setJointMotorControlArray(
                    self.end_effector.body_id, [6, 3, 8, 5, 10], p.POSITION_CONTROL,
                    [
