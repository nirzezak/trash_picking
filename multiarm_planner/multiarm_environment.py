...
class MultiarmEnvironment:
    ...
    def __init__(self, gui=True, visualize=False, ur5_group=None):
        ...
        self.ur5_group = UR5Group(create_ur5s_fn=create_ur5s_fn, collision_distance=0) if ur5_group is None else ur5_group
    ...
    def birrt2(self, goal_configs, target_eff_poses):

        ur5_poses = [ur5.get_pose() for ur5 in self.ur5_group.active_controllers]
        start_configs = [ur5.get_arm_joint_values() for ur5 in self.ur5_group.active_controllers]

        return self.birrt(start_configs, goal_configs, ur5_poses, target_eff_poses)

    def mrdrrt_from_task(self, task, cache_roadmaps=False, num_prm_nodes=50):
    ...
    def mrdrrt2(self, goal_configs, target_eff_poses):

        ur5_poses = [ur5.get_pose() for ur5 in self.ur5_group.active_controllers]
        start_configs = [ur5.get_arm_joint_values() for ur5 in self.ur5_group.active_controllers]

        return self.mrdrrt(start_configs, goal_configs, ur5_poses, target_eff_poses)
